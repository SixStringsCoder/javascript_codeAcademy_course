  ...
  cleanAnalyze(text) {
    ...
    betterWords.forEach(function(freqWord) {
      if (overusedWords.includes(freqWord)) {
        this.setState({stats: { ...this.state.stats, overusedWordsCount: 'overusedWordsCount + 1' }
        });

      }
      if (freqWord === overusedWords[0]) {
        this.setState({stats: { ...this.state.stats, really: 'really + 1' }
        });
    	 } else if (freqWord === overusedWords[1]) {
         this.setState({stats: { ...this.state.stats, very: 'very + 1' }
         });
    	 } else if (freqWord === overusedWords[2]) {
         this.setState({stats: { ...this.state.stats, basically: 'basically + 1' }
         });
    	 };
    });
    ...
    betterWords.forEach((punctuation) => {
      if (punctuation[punctuation.length-1] === "." ||
          punctuation[punctuation.length-1] === "!" ) {
            this.setState({stats: { ...this.state.stats, sentenceCount: 'sentenceCount + 1' }
            });
    	     }
    }); // End of forEach
  } // End of cleanAnalyze
